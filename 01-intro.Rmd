...
Here, we typed `1 + 1`, hit \keys{Enter}, and R produced `2`. The `[1]` means that the result only has one element (the number '2').

In this book, the R output is shown after `\#\#`. Every example can be run by you, simply copy the section (use the text selection tool in Adobe reader), and paste it into the console (with \keys{Ctrl + Enter} on a Windows machine, or \keys{Cmd + Enter} on a Mac).
