...
+=================+=========================================+===============================+==============+
|   *numeric*     |   Any number, including `double`        | `c(1, 12.3491, 10/2, 10^6, pi)`|             |   
|                 | (double precision floating point) and   |                                |             |
|                 | `integer` (whole numbers). In R you very|                                |             |
|                 | rarely have to worry about the exact    |                                |             |
|                 | nature of numeric values.               |                                |             |   
+-----------------+-----------------------------------------+--------------------------------+-------------+
...
the `factor` is different. You can also reorder the levels of a factor by the values of another variable, see the example in Section~\ref{sec:reorder}.
...
> **Try this yourself**
Compare the `summary` of `pupae\_amb` and `pupae\_amb2`, and note the differences.
...
Many functions have an argument `na.rm`, or similar. Refer to the help page
of the function to learn about the various options (if any) for dealing with missing values. For example, see the help pages `?lm` and `?sd`.
...
When working with dataframes, you often want to remove missing values for a particular analysis. We'll use the `pupae` dataset for the following examples. See Section~\ref{sec:pupaedata} for a description of this dataset.
...
There are a few useful functions to print general summaries of a dataframe, to see which variables are included, what types of data they contain, and so on. We already looked at some of these in Section \@ref(dataframes).
...
Finally, there are two very useful functions in the `Hmisc` package. The first, `describe`, is much like `summary`, but offers slightly more sophisticated statistics.
...
### Summarizing dataframes with `dplyr` {#summaryby}
...
elect_long <- gather(elect, Party, Poll, -Date)
head(elect_long,3)
```
...
library(reshape2)
elect_long <- melt(elect, variable.name="Party", value.name="Poll", id.vars="Date")
```
