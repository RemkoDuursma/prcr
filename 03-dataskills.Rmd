...
Finally, there are two very useful functions in the `Hmisc` package (recall how to install and load packages from Section \@ref(packages)). The first, `describe`, is much like `summary`, but offers slightly more sophisticated statistics.
...
group_by(pupae, CO2_treatment, T_treatment) %>%
  dplyr::summarize(Frass_mean = mean(Frass, na.rm=TRUE),
            Frass_sd = sd(Frass, na.rm=TRUE)) %>%
