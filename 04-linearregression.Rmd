...
> **Note** If you run the code above yourself, you will see different output from what is shown here. To get this output in an rmarkdown document, use the `pander` package: `pander::pander(t.test(sales ~ location, data=icecream))
...
```{r}
t.test(weight, mu=0.29) %>% pander::pander(.)
```
...
```{r }
t.test(weight, mu=0.29, alternative="greater") %>%
  pander::pander(.)
```
...
```{r }
wilcox.test(weight, mu=0.29) %>% pander::pander(.)
```
...
prop.test(x=60, n=100, p=0.5) %>% pander::pander(.)
```
...
Notice the four estimated `Coefficients`, these represent the so-called *contrasts*. In this case, `Intercept` represents the mean of the *first* species, `bele`. The next three coefficients are the differences between each species and the first (e.g., species `cofl` has a mean that is \Sexpr{round(fit1$coefficients[[2]],2)} lower than `bele`).
...
> **Try this yourself**
In the above summary of the multiple comparison (`summary(tukey\_Spec)`), the p-values are adjusted for multiple comparisons with the so-called 'single-step method'. To use a different method for the correction (there are many), try the following example:
\begin{verbatim}
summary(tukey_Spec, test=adjusted("Shaffer"))
\end{verbatim}
...
Sometimes there are two (or more) *treatment* factors. The 'age and memory' dataset (see Section~\ref{sec:agemem}) includes the number of words remembered from a list for two age groups and five memory techniques. 
...
```{r interactionplot, fig.width=7, fig.height=5.5, fig.align='center', fig.cap='An interaction plot for the memory data, indicating a strong interaction (because the lines are not parallel).', out.width="0.6\\textwidth"}
...
In section~\ref{sec:multcomp}, we introduced the `glht` package for conducting multiple comparisons testing the significance of differences between levels within a factor. When there is a signficant interaction term, we might want to know under what levels of one factor is the effect of another factor signficant. This can be done easily using functions in the `emmeans` package.
...
We use this method to study the relationship between two continuous variables: a *response* ('y variable') and a *predictor* (or independent variable) ('x variable'). We can use multiple regression to study the relationship between a response and more than one predictor. For example, in the Allometry dataset (see Section~\ref{sec:allomdata}), we might expect that leaf area depends on both tree diameter and height. We test this idea in the following example.

The first step should always be to inspect the data visually. In this case, let's make two simple scatter plots (Fig.~\ref{fig:allomdiamheight}).

```{r allomdiamheight, fig.show='hold',  fig.cap='Leaf area as a function of height and diameter (note the log-log scale).', opts.label="wide"}
...
$$
y = \alpha+\beta_1 x_1 +\beta_2 x_2 +\varepsilon {#eqn:multiplelin}
$$

where $\alpha$ is the intercept, $x_1$ and $x_2$ are the two predictors (height and diameter), andthe *two* slopes are $\beta_1$ and $\beta_2$. We are particularly interested in testing for significant effect of both predictors, which is akin to saying that we are testing the values of the two slopes against zero. We will first use the original scale (i.e. not log-transformed) and perform some diagnostic plots (shown in Fig.~\ref{fig:allomdiamheight2}).

```{r allomdiamheight2, fig.cap='Diagnostic plots for the multiple regression of the Allometry data.', opts.label="wide"}
...
Let's refit the model using log-transformed variables, and check the diagnostic plots (Fig.~\ref{fig:allomdiamheight3}).

```{r allomdiamheight3,opts.label="wide", fig.cap='Diagnostic plots for the multiple regression of the Allometry data, using log-transformed variables.'}
