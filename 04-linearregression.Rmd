...
- methods for very few variables. that's what we prefer, since it is real nice to be able to visualize what is going on. that does not work for many variables
...
g1 <- ggplot(coweeta, aes(x = biomass, y = folmass)) + 
  geom_point() +
  theme_tufte() + 
  #stat_smooth(method = "loess", se = FALSE, col="red2") +
...
Instead what we are interested in is how much variation is explained by each variable in the model. This calculation is complicated by the fact that predictor variables tend to be correlated with each other, so you cannot simply add up the R^2^ due to each predictor separately.
...
The values mean that `weight` explains 21% of the variation, but acceleration only 4%. Note that the sum of these values adds up (nearly precisely) to the R^2^ of the model (check `summary(fit_fuel1)`).
