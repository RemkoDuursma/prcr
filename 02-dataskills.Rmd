...
We first show a few options to read text files into dataframes in R, and then provide the basics of working with dataframes in Section \@ref(dataframes).

In Section \@ref(otherdata), we summarize methods to read in data from other sources (JSON, proprietary formats), and from databases (SQL, NoSQL - Mongodb).
...
Here, we only show how to read rectangular data with the `DBI` package.
...
# Use dbListTables to see what's available.
# Because this is a large database, store it in a vector and inspect the first few
con_tables <- dbListTables(con)
head(con_tables)

# To download an entire table, use this command.
# NOTE: not tested on this database as it is very large!
# Here 'sometable' is one of the tables listed above.
# mydata <- dbReadTable("sometable")

# We can now send SQL queries, and download data.
# Here we grab the last 10 items from the rfam_models table,
# selecting only some columns.
myquery1 <- dbSendQuery(con, "SELECT short_name, long_name, domain FROM rfam_models LIMIT 10")

# This *still* does not download data, which we now do with:
mydata1 <- dbFetch(myquery1)
...
```{block2, type="rmdtry"}
You can run both examples above and download data from the remotely stored databases, both should be operational.
```
...
dfr <- tribble(~`Data type`, ~Description,        ~Example, ~Section,
