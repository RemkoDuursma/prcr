...
Another common and very useful example arises in when reading data from database connections. After we are done with the database, it should be *closed* with `dbDisconnect` (in the DBI package). Sometimes we forget, or code fails, or whatever - leaving extra connections open that can cause various problems (especially in production-ready code).
