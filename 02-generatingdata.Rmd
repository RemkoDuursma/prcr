...
There are many ways to read data into R, but we are going to keep things simple and show only a couple of options. Throughout this book we focus on 'rectangular data', that is, data that can be organized in a table with columns and rows. A data table like this, with individual observations in rows, and various data fields in columns, is called a *data frame*. All example data in this book is provided as dataframes.
...
All example datasets used throughout this book are provided in the R package `lgrdata`. Install it like any other package:
...
If you would like to practice reading data from CSV files, you can also download (nearly) every example dataset from [this page maintained by Western Sydney University](http://www.westernsydney.edu.au/hie/opportunities/training_courses/data_analysis_r/r_course_data_files).



### Reading CSV files {#readcsv}

Suppose you have the file `Allometry.csv' in your working directory, you can read in the file into a dataframe with, 
...
```{r eval=FALSE}
allomsmall <- read.csv("Allometry.", skip=10, nrows=5, header=FALSE)
```
...
```{r }
allometry$volindex <- allometry$diameter^2 * allometry$height
```
...
Another, even more readable approach is to use `mutate` from the `dplyr` package (similar to base R's `transform`, but with a useful advantage).
...
Where `mutate` adds new variables to the dataframe, and it is able to use variables used just before in the same call (note `diameterInch` was created in the same call).
...
A simple summary of the dataframe can be printed with the `summary` function; where we use `pander` (from the `pander` package) to give readable output in rmarkdown (which we assume you use anyway), and indexing (`[,1:3]`) to use the first three columns of `allometry` only.

```{r }
library(pander)
summary(allometry[,1:3]) %>% pander
```
...
allometry <- rename(allometry, spec = species,
                       diam = diameter)
```
...
With `filter`, you can select rows that meet a certain criterion, and columns as well. This example uses the pupae data, see Section \@ref(pupaedata). The last example shows the use of `select` from `dplyr` to keep only certain columns, conveniently with the pipe operator.
