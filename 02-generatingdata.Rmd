...
A simple summary of the dataframe can be printed with the `summary` function; where we use `pander` (from the `pander` package) to give readable output in rmarkdown (which we assume you use anyway), and indexing (`[,1:3]`) to use the first three columns of `allom` only.
