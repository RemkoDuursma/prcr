...
Where `mutate` adds new variables to the dataframe, and it is able to use variables used just before in the same call (note `diameterInch`).
...
Note that for `select`, we habitually make sure to load it from `dplyr` (Hmisc and other packages cause frequent conflicts).
