...
We can use this `function` just like any other in R, for example, let's convert 'weight' to kilograms in the weightloss data (Section~\ref{sec:weightlossdata}).
```{r }
# Read data
data(weightloss)
...
We will return to defining our own functions when we look at applying functions many times to sections of a dataframe (Section~\ref{sec:lapply}).
...
Let's look at an example using the `hydro` data. The data contains water levels of a hydrodam in Tasmania, from 2005 to 2011 (see Section~\ref{sec:hydrodata}).
...
*Note:* On windows (with Adobe reader) If the pdf (\file{Allom plot by species.pdf}) is open, the above will fail. If you try this anyway, close the pdf and try again. You may have to run the command `dev.off`() another time to make sure the device is ready.
...
Let's fit a weibull distribution to the vessel diameter data, separately for the base and apex positions (see Section~\ref{sec:vesseldata}). We will also visualize the results, and print the coefficients of the fit.
