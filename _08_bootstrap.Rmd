...
hfeboot <- bootstrap(hfe_wide, mean(I/C))
...
First we need a function that plots a transparent polygon if we give it three vectors: X, y1 (the upper part of the prediction interval), and y2 (the lower end). Based on  `polygon`, I wrote this simple function which takes three vectors and plots a grey transparent region.

```{r message=FALSE, eval=TRUE}
# For the 'alpha' function which allows transparent colours.
library(scales)
...
augment(fit2, ChickWeight) %>% 
  ...
  geom_line(aes(x = Time, y=weightpred, col=Diet), size=1.5) +
