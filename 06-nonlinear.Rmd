...
This chapter introduces alternatives to linear models, with an emphasis on non-linear regression (Section \@ref(nls)). Very brief introductions to local regression smoothers (loess, Section~\ref{sec:loess}) and quantile regression (Section \ref{sec:rq}), and generalized additive models (Section~\ref{sec:gam}). In each case, we avoid technical descriptions of theory and implementation, and instead illustrate each technique with examples, and many figures.
...
ggplot(hobart, aes(x = Year, y = Time)) +
...
out <- with(iris, 
            expand.grid(Sepal.Width = seq(min(Sepal.Width), max(Sepal.Width), length=50),
                        Sepal.Length = seq(min(Sepal.Length), max(Sepal.Length), length=50))
) 
...
Although we just made a pretty picture, we cannot tell from the figure whether the model provided an adequate fit to the data. For models that include multiple predictors, and you want to use smooth functions, it is recommended to instead use a generalized additive model (see Chapter \@ref(gam)).
...
choat <- read.csv("Choat_precipP50.csv") %>%
  mutate(drought_tolerance = -P50)

# A basic quantile regression
library(quantreg)
choat_rq <- rq(drought_tolerance ~ annualprecip, tau=0.1, data=choat)
...
choat_rq_2 <- rq(drought_tolerance ~ annualprecip, tau=tau_choice, data=choat)
 
# Store coefficients (inspect p, it is a matrix)
p <- coef(choat_rq_2)

# Make base ggplot (points only)
g <- ggplot(choat, aes(x = annualprecip, y = drought_tolerance)) +
  geom_point(col = "dimgrey") +
...
The following code makes Fig. \@ref{fig:fluxplotrq}. Note that in a previous example (Section XXX), we realized that `ggplot2::stat_smooth` could be used to make a similar plot, however, that function does not include an option for quantile regression. The approach shown here is general, and can be adapted to other models as well.
...
choat_rq_95 <- rq(drought_tolerance ~ annualprecip, tau=0.95, data=choat)
...
ggplot(choat, aes(x = annualprecip)) +
...
foot_male <- read.csv("anthropometry.csv") %>%
...
# Loop through desired quantiles, and add a fitted line each time.
